 ...
 * overview.
 * @param <K> key in {@code Map}。  
 * @param <V> value in {@code Map}。
 */
public class LruCache<K, V> {
    ...
    private static final float DISK_PERCENT = 0.75f;
    /**
    ...
    public LruCache(int maxsize) {
        ...
        this.map = new LinkedHashMap<K, V>(0, DISK_PERCENT, true);
    }

    /**
     * @param key {@code key}
     * @return V the value for {@code key} if it exists in the cache or can be
    ...
    /**
     * @return the number of times {@link #get} returned a value.
     * 
     */
	...
	public final synchronized String toString() {
        int accesses = mHitCount + missCount;
        int hitPercent = accesses != 0 ? (PERCENT_RATIO * mHitCount / accesses) : 0;
        return String.format("LruCache[maxSize=%d,hits=%d,misses=%d,hitRate=%d%%]",
                maxSize, mHitCount, missCount, hitPercent);
    }
}
