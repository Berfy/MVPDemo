 ...
 * SwipeMenuListView menuListView = findViewById(R.id.swipeList);
 ...
 * 0xCE)));
 * openItem.setWidth(DeviceUtil.dip2px(context,90));
 ...
 * 0x3F, 0x25)));
 * deleteItem.setWidth(DeviceUtil.dip2px(context,90));
 * deleteItem.setIcon(R.drawable.push);
...
public class SwipeMenuListView extends ListView {
    ...
    private final String TAG = "SwipeMenuListView";
    ...
    private OnTouchScrollListener mOnTouchScrollListener;//列表滑动位置实时监听
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (null != mOnTouchScrollListener) {
            LogUtil.i(TAG, "列表滑动" + "  列表位置" + listViewScorllY());
            mOnTouchScrollListener.scroll(listViewScorllY());
        }
    }
    ...
    public void smoothOpenMenuBy(int position) {
        // if (position >= getFirstVisiblePosition()
        // && position <= getLastVisiblePosition()) {
        if (position >= getFirstVisiblePosition()) {
            View view = getChildAt(position - getFirstVisiblePosition());
