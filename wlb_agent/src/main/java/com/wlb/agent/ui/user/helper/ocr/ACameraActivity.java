package com.wlb.agent.ui.user.helper.ocr;
...
import com.wlb.agent.ui.user.helper.ocr.utils.FileUtil;
import com.wlb.agent.ui.user.helper.ocr.utils.HttpUtil;
import com.wlb.agent.ui.user.helper.ocr.utils.NetUtil;

import java.io.File;
import java.io.IOException;

public class ACameraActivity extends Activity implements Callback {
				...
				if (jpegData != null && jpegData.length > 0) {
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							if (isavilable) {
								result = Scan(OcrConstant.action, jpegData, "jpg");
								Log.d("tag", "<----->>" + result);
								if (result.equals("-2")) {
									result = "连接超时！";
	...
	public static String Scan(String type, byte[] file, String ext) {
		...
		String result = HttpUtil.send(xml, file);
		return result;
	}
